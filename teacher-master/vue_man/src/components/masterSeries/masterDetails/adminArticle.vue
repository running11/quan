...
<div>
    ...
    <div class="content-container" style="margin-left: 0;">
      <div class="artinfo-container content">
        ...
        <div class="article-detailed detailed pull-left">
          ...
          <div class="detailed-txt" v-html="item.content">
						...
						<div class="clist"  v-for = "(items,index) in item.comments.rows">
							...
							<div class="clist-bottom">
								...
								<div class="clist-btns pull-right b-btns">
									<span class="txt a-shelves upper" @click="commentSale(index)" v-if="items.show_status == '已上架'">{{ items.show_status }}</span>
                  <span class="txt a-shelves noUp" @click="commentSale(index)" v-if="items.show_status == '未上架'">{{ items.show_status }}</span>
                  <span class="txt a-shelves lower" @click="commentSale(index)" v-if="items.show_status == '已下架'">{{ items.show_status }}</span>
  ...
  <el-dialog
    ...
    center
    id="speak-dialog">
    <textarea class="speak-textarea comments-textarea" ref = 'comments'></textarea>
  ...
  <el-dialog
    ...
    center
    id="speak-dialog">
    <textarea class="speak-textarea replys-textarea" ref = 'replys'></textarea>
    <div slot="footer" class="dialog-footer">
  ...
  <el-dialog
    ...
    center
    id="del-dialog">
    <span class="text-center">确认要删除这篇文章吗？删除后无法回复，请谨慎操作</span>
...
export default {
  ...
  methods:{
    ...
    goEdit() {
      console.log(11111)
      let aid = this.$route.query.article_id;
      let types = this.$route.query.article_type;
      this.$router.push({path:'/addArticle' , query:{article_id:aid,article_type: types}})
    }
  },
